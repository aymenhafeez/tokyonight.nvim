...
function M.setup()
  ...
  theme.highlights = {
    ...
    CursorLine = { bg = c.bg_highlight_2 }, -- Screen-line at the cursor, when 'cursorline' is set.  Low-priority if foreground (ctermfg OR guifg) is not set.
    ...
    WinSeparator = { fg = c.bg_highlight_2, bold = true }, -- the column separating vertically split windows
    ...
    -- documentation.
    LspReferenceText = { bg = c.doc_hl }, -- used for highlighting "text" references
    LspReferenceRead = { bg = c.doc_hl  }, -- used for highlighting "read" references
    LspReferenceWrite = { bg = c.doc_hl }, -- used for highlighting "write" references
